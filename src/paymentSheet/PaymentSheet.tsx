...
// Several installed bank apps: show the ones we detected (up to three, overlapping)
// instead of a generic Fonepay tile, so the row says "your banks" at a glance.
function BankLogoStack({ banks, extra, colors, apiBaseUrl }: { banks: Bank[]; extra: number; colors: PaymentSheetColors; apiBaseUrl?: string }) {
  return <View style={[styles.logoStack, { width: 40 + (banks.length - 1) * 16 + (extra > 0 ? 18 : 0) }]}>
    {banks.map((bank, index) => <View key={bank.swift} style={[styles.logoStackItem, { left: index * 16, zIndex: banks.length - index, borderColor: colors.border }]}>{bank.logo ? <Image resizeMode="contain" source={{ uri: mobileAssetUrl(bank.logo, apiBaseUrl) }} style={styles.logo} /> : <Text style={{ color: "#374151", fontWeight: "700", fontSize: 11 }}>{bank.short}</Text>}</View>)}
    {extra > 0 ? <View style={[styles.logoStackItem, styles.logoStackMore, { left: banks.length * 16, borderColor: colors.border }]}><Text style={{ color: colors.mutedText, fontWeight: "700", fontSize: 11 }}>+{extra}</Text></View> : null}
  </View>;
}
...
// "Nabil, Siddhartha and 2 more" — the detected banks named in the row subtitle.
function bankNames(banks: Bank[]): string {
...
export function PaymentSheet({ visible, session, publishableKey, config, returnUrl, appearance, onComplete, onCancel, onError }: PaymentSheetProps) {
  ...
  const providerName = knownProvider === "khalti" ? "Khalti" : knownProvider === "esewa" ? "eSewa" : knownProvider === "fonepay" ? "Fonepay" : "the provider";
  const providerTitle = knownProvider === "khalti" ? "Khalti payment still active" : knownProvider === "esewa" ? "eSewa payment still active" : knownProvider === "fonepay" ? "Fonepay payment still active" : "Payment still active";
...
const styles = StyleSheet.create({
  ...
  logoStack: { height: Platform.OS === "android" ? 34 : 40, position: "relative" },
  logoStackItem: { position: "absolute", top: 0, width: Platform.OS === "android" ? 34 : 40, height: Platform.OS === "android" ? 34 : 40, borderRadius: Platform.OS === "android" ? 9 : 10, padding: 5, backgroundColor: "#FFFFFF", borderWidth: StyleSheet.hairlineWidth, justifyContent: "center", alignItems: "center" },
  logoStackMore: { backgroundColor: "#F3F4F6" },
